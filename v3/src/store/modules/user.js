...
export function filterAsyncRoutes(routes,config) {
    ...
    routes.forEach(route => {
        ...
        let tmp={
            path: route.key,
            component: () => import(/* webpackChunkName: "[request]" */ `@/views/pages/${route.component}/Index`),
            name: route.key,
        ...
        if(route.relation&&route.relation.length>0){
            route.relation.forEach(element => {
                tmp={
                    ...
                    component: () => import(/* webpackChunkName: "[request]" */ `@/views/pages/${route.component}/${element.component}`),
                    name: element.key,
